...
export default class Video extends Component {
  ...
  save = async (options?) => {
    const re = await NativeModules.VideoManager.save(options, findNodeHandle(this._root));
    return re;
  }
  ...
  render() {
    ...
    if (!uri) {
      console.warn('aplayer: Trying to load empty source.');
    }
    ...
    const RCTVideoInstance = this.getViewManagerConfig('RCTAPlayer');
...
const RCTVideo = requireNativeComponent('RCTAPlayer', Video, {
  nativeOnly: {
    src: true,
    seek: true,
    fullscreen: true,
